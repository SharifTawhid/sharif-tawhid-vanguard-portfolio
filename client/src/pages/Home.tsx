...
const certificates = [
  { number: "01", title: "Marketing Explorer", image: "/manus-storage/marketing-explorer-certificate.png", download: "/manus-storage/marketing-explorer-certificate.pdf", label: "Marketing Explorer Job Simulation", skills: "Creative Strategy / Performance Analysis / Customer & Product Marketing / Digital Marketing", description: "A hands-on exploration of how creative thinking, performance analysis and customer understanding connect to modern marketing decisions.", folder: "forage" },
  { number: "02", title: "Product Management", image: "/manus-storage/product-management-certificate.png", download: "/manus-storage/product-management-certificate.pdf", label: "Product Management Job Simulation", skills: "Product Performance / Stakeholder Presentation Planning", description: "An opportunity to approach growth from a product perspective — understanding performance, structuring insights and communicating decisions to stakeholders.", folder: "forage" },
  { number: "03", title: "Omnichannel Marketing", image: "/manus-storage/omnichannel-marketing-certificate.png", download: "/manus-storage/omnichannel-marketing-certificate.pdf", label: "Omnichannel Marketing Job Simulation", skills: "Integrated Marketing Planning / Digital Retail Experience / Local Ambassador Strategy / Data Analysis", description: "A practical exploration of how digital, customer experience, local activation and data can work together inside an integrated marketing system.", folder: "forage" },
  { number: "04", title: "Influencer Marketing", image: "/manus-storage/upgrad-influencer-marketing-certificate.jpeg", download: "/manus-storage/upgrad-influencer-marketing-certificate.jpeg", label: "Influencer Marketing from a Brand’s Perspective", skills: "Brand Perspective / Influencer Marketing / Campaign Thinking", description: "A focused certification exploring how brands can evaluate influencer marketing through strategy, audience fit and commercial context.", folder: "certifications" },
];
const reports = [
...
const proofScreens = [
  ...
  ["Payrashopping Meta Ads overview", "/manus-storage/proof-payrashopping-meta-ads-overview.jpeg"],
  ["Shopify dashboard overview", "/manus-storage/proof-shopify-dashboard-overview.jpeg"],
];
const releaseBase = "https://github.com/roman33-droid/sharif-tawhid-vanguard-portfolio/releases/download/portfolio-assets-v1";
const rawBase = "https://raw.githubusercontent.com/roman33-droid/sharif-tawhid-vanguard-portfolio-assets/main";
const assetFile = (path: string) => path.split("/").pop()!.replace(/_[a-z0-9]{8}(?=\.[^.]+$)/i, "");
const releaseHref = (path: string) => `${releaseBase}/${assetFile(path)}`;
const rawHref = (folder: string, path: string) => `${rawBase}/${folder}/${assetFile(path)}`;
...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      ...
      <section className="forage-section section-pad"><div className="section-intro"><p className="section-index">02.5 / Practical Work & Certification</p><h2>Practical work.<br /><em>Not just theory.</em></h2><p className="section-desc">Industry-focused simulations and certifications completed through international platform collaborations connecting learners with practical work experiences from leading organizations.</p></div><div className="forage-lede"><p>These experiences expanded my work beyond campaign execution — into strategy, analysis, product thinking, customer experience, integrated marketing and brand perspective.</p><span>INTERNATIONAL PLATFORM COLLABORATION / Practical Work & Certification</span></div><div className="forage-grid">{certificates.map((certificate) => <article className="forage-card" key={certificate.title}><div className="forage-card-meta"><span>{certificate.number} — {certificate.title}</span><a href={releaseHref(certificate.download)} download target="_blank" rel="noreferrer">Download certificate <ArrowUpRight size={14} /></a></div><a className="certificate-image" href={releaseHref(certificate.download)} target="_blank" rel="noreferrer"><img src={rawHref(certificate.folder, certificate.image)} alt={`${certificate.label} certificate`} loading="lazy" /></a><div className="forage-card-copy"><h3>{certificate.label}</h3><p className="forage-skills"><strong>Practical experience across:</strong> {certificate.skills}</p><p>{certificate.description}</p></div></article>)}</div><div className="forage-close"><p>These weren’t simply certificates collected for a résumé.</p><p>They represent completed practical work and certifications designed around real-world professional tasks, adding another layer to my experience across marketing, performance, strategy, analytics, product thinking and brand building.</p></div></section>
